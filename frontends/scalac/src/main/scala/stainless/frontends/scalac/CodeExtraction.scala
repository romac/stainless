...
trait CodeExtraction extends ASTExtractors {
  ...
  implicit def scalaPosToInoxPos(p: global.Position): inox.utils.Position = {
    if (p == NoPosition) {
      ...
      inox.utils.OffsetPosition(p.line, p.column, p.point, p.source.file.file)
    }
  }
  ...
  private def annotationsOf(sym: Symbol, ignoreOwner: Boolean = false): Seq[xt.Flag] = {
    getAnnotations(sym, ignoreOwner).map {
      case (name, args) =>
  ...
  private def extractClass(cd: ImplDef): (xt.ClassDef, Seq[xt.FunDef]) = {
    ...
    val parents = cd.impl.parents.flatMap(
      p =>
        p.tpe match {
    ...
    val constructorOpt = cd.impl.children
      .find {
    ...
    val vds = cd.impl.children.collect {
      case vd: ValDef
          if !sym.isImplicit && !vd.symbol.isAccessor && vd.symbol.isCaseAccessor && vd.symbol.isParamAccessor =>
        vd
    ...
    val fields = vds map { vd =>
      ...
      val tpe =
        if (isIgnored) xt.IntegerType().setPos(vd.pos)
    ...
    for (d <- cd.impl.body) d match {
      ...
      case ExCaseClassSyntheticJunk() | ExConstructorDef() | ExLazyFieldDef() =>
      // ignore
  ...
  private def extractFunction(
    ...
    val paramsMap = (vparams.map(_.symbol) zip newParams).map {
      case (s, vd) =>
        s -> (if (s.isByNameParam) () => xt.Application(vd.toVariable, Seq()).setPos(vd.toVariable)
              else () => vd.toVariable)
    }.toMap
  ...
  private def extractPattern(p: Tree, binder: Option[xt.ValDef] = None)(
      implicit dctx: DefContext
  ): (xt.Pattern, DefContext) = p match {
    case b @ Bind(name, t @ Typed(pat, tpt)) =>
      val vd = xt
        .ValDef(FreshIdentifier(name.toString), extractType(tpt), annotationsOf(b.symbol, ignoreOwner = true))
        .setPos(b.pos)
    ...
    case b @ Bind(name, pat) =>
      val vd = xt
        .ValDef(FreshIdentifier(name.toString), extractType(b), annotationsOf(b.symbol, ignoreOwner = true))
        .setPos(b.pos)
      val pctx = dctx.withNewVar(b.symbol, () => vd.toVariable)
    ...
    (tr match {
      ...
      case ExNotEquals(l, r) =>
        xt.Not(((extractTree(l), extractType(l), extractTree(r), extractType(r)) match {
          case (bi @ xt.BVLiteral(_, _, _), _, e, xt.IntegerType()) =>
            xt.Equals(xt.IntegerLiteral(bi.toBigInt).setPos(l.pos), e)
          case (e, xt.IntegerType(), bi @ xt.BVLiteral(_, _, _), _) =>
            xt.Equals(e, xt.IntegerLiteral(bi.toBigInt).setPos(r.pos))
              ...
              case tpe =>
                (tpe, sym.name.decode.toString, args) match {
                  ...
                  case (xt.MapType(_, xt.ClassType(_, Seq(to))), "+", Seq(rhs)) =>
                    ...
                    xt.MapUpdated(
                      extractTree(lhs),
                      xt.TupleSelect(value, 1).setPos(tr.pos),
                      xt.ClassConstructor(
                          xt.ClassType(getIdentifier(someSymbol), Seq(to)).setPos(tr.pos),
  ...
  )(lhs0: Tree, rhs0: Tree)(implicit dctx: DefContext): xt.Expr = {
  ...
  private def injectCastsImpl(
      ctor: (xt.Expr, xt.Expr) => xt.Expr
  )(lhs0: Tree, rhs0: Tree, shift: Boolean)(implicit dctx: DefContext): xt.Expr = {
