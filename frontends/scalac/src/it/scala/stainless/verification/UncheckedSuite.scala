...
trait UncheckedSuite extends ComponentTestSuite {
  ...
  override def configurations = super.configurations.map { seq =>
    Seq(optFailEarly(true), inox.solvers.optCheckModels(false)) ++ seq
  }
  ...
  testAll("verification/unchecked") { (analysis, _) =>
    ...
    assert(
      report.totalInvalid > 0 || report.totalUnknown > 0,
...
class SMTZ3UncheckedSuite extends UncheckedSuite {
  override def configurations = super.configurations.map { seq =>
    inox.optSelectedSolvers(Set("smt-z3")) +: seq
  }
}

class SMTCVC4UncheckedSuite extends UncheckedSuite {
  override def configurations = super.configurations.map { seq =>
    inox.optSelectedSolvers(Set("smt-cvc4")) +: seq
  }
}
