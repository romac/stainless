...
class CodeExtraction(inoxCtx: inox.Context, cache: SymbolsContext)(implicit val ctx: Context) extends ASTExtractors {
    ...
    scala.util
      .Try({
        if (!p.exists) {
          ...
          inox.utils.OffsetPosition(sp.line + 1, sp.column + 1, sp.point, ctx.source.file.file)
        }
      })
      .toOption
      .getOrElse(inox.utils.NoPosition)
  ...
  private def extractClass(td: tpd.TypeDef): (xt.ClassDef, Seq[xt.FunDef]) = {
    ...
    for (d <- template.body) d match {
      ...
      case t @ ExFunctionDef(fsym, _, _, _, _) if hasIgnoredFields && (isCopyMethod(fsym) || isDefaultGetter(fsym)) =>
  ...
  private def extractFunction(
    ...
    val (newParams, fctx0) = vdefs.foldLeft((Seq.empty[xt.ValDef], nctx)) {
      case ((params, dctx), param) =>
  ...
  private def typeParamSymbols(tdefs: Seq[tpd.TypeDef]): Seq[Symbol] =
    tdefs.flatMap(_.tpe match {
      case tp @ TypeRef(_, _) =>
        Some(tp.symbol)
      case t =>
  ...
  private def extractTypeParams(syms: Seq[Symbol])(implicit dctx: DefContext): Seq[xt.TypeParameter] = {
    syms
      .foldLeft((dctx, Seq[xt.TypeParameter]())) {
        case ((dctx, tparams), sym) =>
          val variance =
  ...
  private def extractPattern(p: tpd.Tree, binder: Option[xt.ValDef] = None)(
      implicit dctx: DefContext
  ): (xt.Pattern, DefContext) = p match {
    ...
    case t @ Typed(UnApply(f, _, pats), tp) =>
      ...
      if (sym.owner.exists && sym.owner.is(Synthetic) &&
        ...
        val id = getIdentifier(sym)
        val tps = f match {
          case TypeApply(un, tps) => tps map extractType
          case _ => Seq.empty
        }
  ...
  private def extractBlock(es: List[tpd.Tree])(implicit dctx: DefContext): xt.Expr = {
    ...
    val (vds, vctx) = es
      ...
      .foldLeft((Map.empty[Symbol, xt.ValDef], fctx)) {
        case ((vds, dctx), (sym, name, tpt)) =>
          if (sym is Mutable) {
            val vd = xt
              .VarDef(FreshIdentifier(name.toString), extractType(tpt)(dctx), annotationsOf(sym, ignoreOwner = true))
              .setPos(sym.pos)
            (vds + (sym -> vd), dctx.withNewMutableVar((sym, () => vd.toVariable)))
          } else {
            val vd = xt
              .ValDef(FreshIdentifier(name.toString), extractType(tpt)(dctx), annotationsOf(sym, ignoreOwner = true))
              .setPos(sym.pos)
  ...
  private def extractTree(tr: tpd.Tree)(implicit dctx: DefContext): xt.Expr =
    (tr match {
      case Block(Seq(dd @ DefDef(_, _, Seq(vparams), _, _)), ExUnwrapped(Closure(Nil, call, targetTpt)))
          if call.symbol == dd.symbol =>
        val vds = vparams map (
            vd =>
              xt.ValDef(
      ...
      case Ident(_) if tr.tpe.signature.resSig.toString startsWith "scala.collection.immutable.Nil" =>
        outOfSubsetError(
          tr.pos,
          "Scala's List API is no longer extracted. Make sure you import stainless.lang.collection.List that defines supported List operations."
        )
      ...
      case t @ ExHolds(body, Apply(ExSymbol("stainless", "lang", "package$", "because"), Seq(proof))) =>
        ...
        val post =
          xt.Lambda(Seq(vd), xt.And(Seq(extractTreeOrNoTree(proof), vd.toVariable)).setPos(tr.pos)).setPos(tr.pos)
        xt.Ensuring(extractTreeOrNoTree(body), post).setPos(post)

      case t @ ExHolds(body, proof) =>
        val vd = xt.ValDef.fresh("holds", xt.BooleanType().setPos(tr.pos)).setPos(tr.pos)
        val post = xt.Lambda(Seq(vd), xt.And(Seq(extractTree(proof), vd.toVariable)).setPos(tr.pos)).setPos(tr.pos)
        xt.Ensuring(extractTreeOrNoTree(body), post).setPos(post)
      ...
      case t @ ExComputes(body, expected) =>
        ...
        val post =
          xt.Lambda(Seq(vd), xt.Equals(vd.toVariable, extractTreeOrNoTree(expected)).setPos(tr.pos)).setPos(tr.pos)
        xt.Ensuring(extractTreeOrNoTree(body), post).setPos(post)
      ...
      case t @ Apply(
        ...
        rest match {
          case Seq() =>
            ...
            xt.Let(
              vd,
              extractTreeOrNoTree(str),
              xt.SubString(vd.toVariable, start, xt.StringLength(vd.toVariable).setPos(t.pos)).setPos(t.pos)
            )
          case Seq(endTree) =>
      ...
      case Apply(Select(l, nme.NE), Seq(r)) =>
        xt.Not(((extractTree(l), extractType(l), extractTree(r), extractType(r)) match {
          case (lit @ xt.BVLiteral(_, _, _), _, e, xt.IntegerType()) =>
            xt.Equals(xt.IntegerLiteral(lit.toBigInt).copiedFrom(lit), e)
          case (e, xt.IntegerType(), lit @ xt.BVLiteral(_, _, _), _) =>
            xt.Equals(e, xt.IntegerLiteral(lit.toBigInt).copiedFrom(lit))
          case _ => injectCasts(xt.Equals)(l, r)
        }).setPos(tr.pos))

      case Apply(Select(l, nme.EQ), Seq(r)) =>
        (extractTree(l), extractType(l), extractTree(r), extractType(r)) match {
          case (lit @ xt.BVLiteral(_, _, _), _, e, xt.IntegerType()) =>
            xt.Equals(xt.IntegerLiteral(lit.toBigInt).copiedFrom(lit), e)
          case (e, xt.IntegerType(), lit @ xt.BVLiteral(_, _, _), _) =>
            xt.Equals(e, xt.IntegerLiteral(lit.toBigInt).copiedFrom(lit))
      ...
      case Select(Apply(ExSymbol("scala", "Predef$", s), Seq(lhs)), ExNamed("size")) if s.toString contains "Array" =>
        ...
        rec match {
          ...
          case Some(lhs) =>
            extractType(lhs) match {
              ...
              case tpe =>
                (tpe, sym.name.decode.toString, args) match {
                  case (xt.StringType(), "+", Seq(rhs)) => xt.StringConcat(extractTree(lhs), extractTree(rhs))
                  case (xt.IntegerType() | xt.BVType(_, _) | xt.RealType(), "+", Seq(rhs)) =>
                    injectCasts(xt.Plus)(lhs, rhs)
                  ...
                  case (xt.SetType(b), "-", Seq(rhs)) =>
                    xt.SetDifference(extractTree(lhs), xt.FiniteSet(Seq(extractTree(rhs)), b).setPos(tr.pos))
                  ...
                  case (xt.ArrayType(_), "updated", Seq(index, value)) =>
                    xt.ArrayUpdated(extractTree(lhs), extractTree(index), extractTree(value))
                  case (xt.ArrayType(_), "update", Seq(index, value)) =>
                  ...
                  case (xt.MapType(from, xt.ClassType(_, Seq(to))), "+", Seq(rhs)) =>
                    val vd =
                      xt.ValDef(FreshIdentifier("x", true), xt.TupleType(Seq(from, to)).setPos(tr.pos)).setPos(tr.pos)
                    xt.Let(
                      vd,
                      extractTree(rhs),
                      xt.MapUpdated(
                          extractTree(lhs),
                          xt.TupleSelect(vd.toVariable, 1).setPos(tr.pos),
                          xt.ClassConstructor(
                              xt.ClassType(getIdentifier(someSymbol), Seq(to)).setPos(tr.pos),
                  ...
                  case (xt.MapType(_, xt.ClassType(_, Seq(to))), "++", Seq(rhs)) =>
                    extractTree(rhs) match {
                      case xt.FiniteMap(pairs, default, keyType, valueType) =>
                        pairs.foldLeft(extractTree(lhs)) {
                          case (map, (k, v)) =>
  ...
  private def injectCasts(
      ctor: (xt.Expr, xt.Expr) => xt.Expr
  )(lhs0: tpd.Tree, rhs0: tpd.Tree)(implicit dctx: DefContext): xt.Expr = {
  ...
  /**
    ...
    */
  private def injectCastsForShift(
      ctor: (xt.Expr, xt.Expr) => xt.Expr
  )(lhs0: tpd.Tree, rhs0: tpd.Tree)(implicit dctx: DefContext): xt.Expr = {
    injectCastsImpl(ctor)(lhs0, rhs0, shift = true)
  }

  private def injectCastsImpl(
      ctor: (xt.Expr, xt.Expr) => xt.Expr
  )(lhs0: tpd.Tree, rhs0: tpd.Tree, shift: Boolean)(implicit dctx: DefContext): xt.Expr = {
    ...
    val widen32 = { (e: xt.Expr) =>
      xt.BVWideningCast(e, xt.BVType(true, 32).copiedFrom(e)).copiedFrom(e)
    }
    val widen64 = { (e: xt.Expr) =>
      xt.BVWideningCast(e, xt.BVType(true, 64).copiedFrom(e)).copiedFrom(e)
    }
  ...
  private def injectCast(ctor: xt.Expr => xt.Expr)(e0: tpd.Tree)(implicit dctx: DefContext): xt.Expr = {
    ...
    val widen32 = { (e: xt.Expr) =>
      xt.BVWideningCast(e, xt.Int32Type().copiedFrom(e)).copiedFrom(e)
    }
  ...
  private def extractType(tpt: Type)(implicit dctx: DefContext, pos: Position): xt.Type =
    etCache.getOrElseUpdate(
      tpt,
      (tpt match {
        ...
        case tr: TypeRef if isScalaSetSym(tr.symbol) =>
          outOfSubsetError(
            pos,
            "Scala's Set API is no longer extracted. Make sure you import stainless.lang.Set that defines supported Set operations."
          )

        case tr: TypeRef if isScalaListSym(tr.symbol) =>
          outOfSubsetError(
            pos,
            "Scala's List API is no longer extracted. Make sure you import stainless.lang.collection.List that defines supported Map operations."
          )

        case tr: TypeRef if isScalaMapSym(tr.symbol) =>
          outOfSubsetError(
            pos,
            "Scala's Map API is no longer extracted. Make sure you import stainless.lang.Map that defines supported Map operations."
          )
        ...
        case tt @ TypeRef(_, _) if tt.classSymbol.exists =>
          ...
          xt.ClassType(getIdentifier(sym), sym.typeParams.map { sym =>
