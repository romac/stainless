...
object FlatMap {
  ...
  def associative_append_lemma_induct[T](l1: List[T], l2: List[T], l3: List[T]): Boolean = {
    l1 match {
      ...
      case Cons(head, tail) =>
        associative_append_lemma(l1, l2, l3) because associative_append_lemma_induct(tail, l2, l3)
    }
  }.holds
  ...
  def associative_lemma_induct[T, U, V](
      list: List[T],
      flist: List[U],
      glist: List[V],
      f: T => List[U],
    ...
    associative_lemma(list, f, g) because {
      append(glist, flatMap(append(flist, flatMap(list, f)), g)) == append(
        append(glist, flatMap(flist, g)),
        flatMap(list, (x: T) => flatMap(f(x), g))
      ) because
        (glist match {
