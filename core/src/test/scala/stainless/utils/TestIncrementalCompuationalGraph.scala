...
class TestIncrementalComputationalGraph extends FunSuite {
  ...
  testSequence(
    "foo1.e",
    Seq(bar, bar, main, main, foo1, bar),
  ...
  testSequence(
    "recursive.c",
    Seq(recY1, recX1, recZ, ext, recX2),
    Seq(
      Set(),
      Set(recY1, recX1),
  ...
  test("removal of nodes & compute = false") {
    ...
    assert(
      g.update("a", "a", Set.empty, compute = true) == Some(Set("a", "b")),
      "a & b should be re-computed, but not c"
    )
    ...
    assert(
      g.update("e", "e", Set("b"), compute = true) == Some(Set("a", "b", "e")),
      "e should be computed with its deps."
    )
  ...
  test("freeze") {
    ...
    assert(
      g.update("top_inv", "top_inv$0", Set(), compute = true) == Some(Set("foobar", "Top", "Bottom", "top_inv")),
      "ready"
    )
  ...
  test("extra regression 01") {
    ...
    assert(
      g.update("prop", "prop$0", Set("Top"), compute = true) ==
        Some(Set("prop", "inv", "Top", "Bottom")),
      "prop"
    )
    assert(
      g.update("foo", "foo$0", Set("prop", "Top"), compute = true) ==
        Some(Set("foo", "prop", "Top", "Bottom", "inv")),
      "foo v1"
    )
    g.freeze()
    assert(g.update("Top", "Top$0", Set("Bottom", "inv", "prop"), compute = false) == None, "Top")
