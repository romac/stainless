...
trait Registry {
  ...
  final def update(classes: Seq[xt.ClassDef], functions: Seq[xt.FunDef]): Seq[xt.Symbols] = synchronized {
    if (hasPersistentCache) {
      ...
      deferredNodes ++= classes map { cd =>
        cd.id -> Left(cd)
      }
      deferredNodes ++= functions map { fd =>
        fd.id -> Right(fd)
      }
      Seq()
    ...
    try {
      ...
      nodes foreach {
        case (id, (node, deps)) =>
          ...
          val hashes = deps.toSeq map { dep =>
            getCF(nodes(dep)._1).hashCode
          }
  ...
  private def isOfInterest(node: NodeValue, deps: Set[Identifier]): Boolean = {
    ...
    val result = persistentCache
      .flatMap { cache =>
        cache.get(id).map {
          case (cachedCf, checked, hash) =>
  ...
  private def updateImpl(classes: Seq[xt.ClassDef], functions: Seq[xt.FunDef]): Seq[xt.Symbols] = {
    def isReady(cd: xt.ClassDef): Boolean = getTopLevels(classes, cd) match {
      ...
      case Some(tops) =>
        tops forall { top =>
          top.flags contains xt.IsSealed
        } // All parents must be sealed.
      case None =>
    ...
    val funDB = (functions map { fd =>
      fd.id -> fd
    }).toMap
    val deferredFDs = openCDs flatMap { cd =>
      invariants(cd) ++ methods(cd)
    } map funDB
    val readyFDs = functions filterNot { deferredFDs contains _ }

    knownOpenClasses ++= openCDs map { cd =>
      cd.id -> cd
    }
  ...
  private def process(classes: Seq[xt.ClassDef], functions: Seq[xt.FunDef]): Seq[xt.Symbols] = {
    ...
    val newNodes: Seq[(Identifier, NodeValue, Set[Identifier])] =
      (classes map { cd =>
        (cd.id, Left(cd): NodeValue, computeAllDirectDependencies(cd))
      }) ++
        (functions map { fd =>
          (fd.id, Right(fd): NodeValue, computeDirectDependencies(fd))
        })
  ...
  private def computeClusters(classes: Seq[xt.ClassDef]): ClusterMap = {
    ...
    classes foreach { cd =>
      ...
      tops foreach { top =>
        toplevels.getOrElseUpdate(top, MutableSet.empty) += cd.id
      }
  ...
  /**
    ...
    */
  private def getTopLevels(classes: Seq[xt.ClassDef], cd: xt.ClassDef): Option[Set[xt.ClassDef]] =
    try {
  ...
  private def forceGetTopLevels(classes: Seq[xt.ClassDef], cd: xt.ClassDef): Set[xt.ClassDef] =
    try {
  ...
  private case class IncompleteHierarchy(cd: Identifier, parent: Identifier, classes: Seq[xt.ClassDef])
      extends Throwable
  private def getTopLevelsImpl(classes: Seq[xt.ClassDef], cd0: xt.ClassDef): Set[xt.ClassDef] = {
    ...
    if (cd0.parents.isEmpty) Set.empty
      ...
      getDirects(cd0) flatMap { p =>
        if (p.parents.isEmpty) Set(p) else getTopLevelsImpl(classes, p)
      }
  }
