...
trait Definitions extends extraction.Trees { self: Trees =>
  ...
  case class Outer(fd: FunDef)
      extends FunAbstraction(fd.id, fd.tparams, fd.params, fd.returnType, fd.fullBody, fd.flags) {
    setPos(fd)
  ...
  case class Inner(fd: LocalFunDef)
      extends FunAbstraction(fd.id, fd.tparams, fd.params, fd.returnType, fd.fullBody, fd.flags) {
    setPos(fd)
  ...
  trait AbstractSymbols extends super.AbstractSymbols with SymbolOps with TypeOps { self0: Symbols =>
