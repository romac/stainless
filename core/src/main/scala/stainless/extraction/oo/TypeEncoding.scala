...
trait TypeEncoding
  ...
  private[this] class CachedID[T](builder: T => Identifier) extends utils.ConcurrentCached[T, Identifier](builder)
  ...
  private[this] def getRefField(e: t.Expr, id: Identifier): t.Expr = e match {
    ...
    case _ =>
      let(("x" :: ref.copiedFrom(e)).copiedFrom(e), e) { x =>
  ...
  private[this] def erasedBy(tpe: s.Type)(implicit scope: Scope): s.Type =
    s.typeOps.postMap {
  ...
  private[this] def wrap(e: t.Expr, tpe: s.Type)(implicit scope: Scope): t.Expr =
    (tpe match {
      case s.AnyType() => e
  ...
  private[this] def unwrap(e: t.Expr, tpe: s.Type)(implicit scope: Scope): t.Expr =
    (tpe match {
      case s.AnyType() => e
      case s.ClassType(id, tps) => e
      case tp: s.TypeParameter => e
    ...
    ((e, tpe.getType(scope.symbols), expected.getType(scope.symbols)) match {
      ...
      case (_, s.ADTType(id1, tps1), s.ADTType(id2, tps2)) if id1 == id2 =>
        t.FunctionInvocation(
          convertID(id1),
          (tps1 ++ tps2).map(tp => scope.transform(tp)),
          e +: (tps1 zip tps2 flatMap {
            case (tp1, tp2) =>
              Seq(
      ...
      case (t.Lambda(params, body), s.FunctionType(from1, to1), s.FunctionType(from2, to2)) =>
        ...
        val unifiedParams = newParams zip (from1 zip from2) map {
          case (vd, (tp1, tp2)) => convert(vd.toVariable, tp2, tp1)
        }
        val newBody =
          convert(t.exprOps.replaceFromSymbols((params.map(_.toVariable) zip unifiedParams).toMap, body), to1, to2)
      ...
      case (t.LargeArray(elems, dflt, size, base), s.ArrayType(b1), s.ArrayType(b2)) =>
        t.LargeArray(elems.map(p => p._1 -> convert(p._2, b1, b2)), convert(dflt, b1, b2), size, scope.transform(b2))
      ...
      case (_, s.ArrayType(b1), s.ArrayType(b2)) =>
        choose(t.ValDef(FreshIdentifier("res"), scope.transform(expected), Seq(t.Unchecked)).copiedFrom(e)) { res =>
          forall(("i" :: t.Int32Type().copiedFrom(e)).copiedFrom(e)) { i =>
            t.Equals(
      ...
      case (t.FiniteMap(pairs, dflt, _, _), s.MapType(f1, t1), s.MapType(f2, t2)) =>
        t.FiniteMap(
          pairs.map(p => convert(p._1, f1, f2) -> convert(p._2, t1, t2)),
          convert(dflt, t1, t2),
          scope.transform(f2),
          scope.transform(t2)
        )
  ...
  private[this] def instanceOf(e: t.Expr, in: s.Type, tpe: s.Type)(implicit scope: Scope): t.Expr =
    ((in, tpe) match {
      ...
      case (s.ADTType(id1, tps1), s.ADTType(id2, tps2)) if id1 == id2 =>
        t.FunctionInvocation(
          instanceID(id1),
          tps1 map (tp => scope.transform(tp)),
          e +: (tps1 zip tps2 map {
            case (tp1, tp2) =>
      ...
      case (ft1 @ (_: s.FunctionType | _: s.PiType), ft2 @ (_: s.FunctionType | _: s.PiType)) =>
        ...
        val paramsCond = t
          .andJoin(nparams1 zip nparams2 map {
            case (vd1, vd2) =>
        ...
        val toCond = (nparams1 zip nparams2).foldRight(instanceOf(app, to1, to2)) {
          case ((vd1, vd2), e) =>
      ...
      case (tt1 @ (_: s.TupleType | _: s.SigmaType), tt2 @ (_: s.TupleType | _: s.SigmaType)) =>
        ...
        val paramsCond = t
          .andJoin((nparams1 zip nparams2).zipWithIndex map {
            case ((vd1, vd2), i) =>
        ...
        val wrappedCond1 = nparams1.zipWithIndex.foldRight(innerCond) {
          case ((vd, i), body) =>
        ...
        val toCond = (nparams1 zip nparams2).zipWithIndex.foldRight(wrappedCond1) {
          case (((vd1, vd2), i), body) =>
      ...
      case (s.ArrayType(b1), s.ArrayType(b2)) =>
        forall(("i" :: Int32Type().copiedFrom(e)).copiedFrom(e)) { i =>
          ((
            ...
            instanceOf(
              t.ArraySelect(e, i).copiedFrom(e),
              b1,
              b2
            )
          )).copiedFrom(e)
  ...
  private[this] val classInstanceCache = new ExtractionCache[s.ClassDef, t.FunDef]({ (cd, context) =>
    ClassKey(cd) + SetKey(cd.children(context.symbols).toSet)
  })
  ...
  private[this] def unapplyAny(implicit context: TransformerContext): t.FunDef =
    unapplyAnyCache.cached(context.symbols, context) {
  ...
  private[this] def sortConvert(id: Identifier)(implicit context: TransformerContext): t.FunDef = {
    ...
    sortConvertCache.cached(sort, context) {
      ...
      val conversions = sort.typed(in).constructors zip sort.typed(out).constructors map {
        case (ci, co) =>
          (x.toVariable is ci.id, C(ci.id)(tout: _*)(ci.fields zip co.fields map {
            case (vi, vo) =>
      ...
      new t.FunDef(
        convertID(id),
        (tin ++ tout).map(t.TypeParameterDef(_)),
        x +: fs,
        T(id)(tout: _*),
        fullBody,
        Seq(t.Unchecked, t.Synthetic)
  ...
  protected class Scope protected (
  ...
  )(implicit val context: TransformerContext)
    ...
    override def transform(tp: s.Type): t.Type = tp match {
      ...
      case s.NothingType() =>
        refinement(("x" :: ref.copiedFrom(tp)).copiedFrom(tp)) { x =>
          t.Annotated(t.BooleanLiteral(false).copiedFrom(tp), Seq(t.Unchecked)).copiedFrom(tp)
        }.copiedFrom(tp)

      case ct: s.ClassType =>
        refinement(("x" :: ref.copiedFrom(tp)).copiedFrom(tp)) { x =>
          t.Annotated(instanceOf(x, s.AnyType().copiedFrom(tp), ct), Seq(t.Unchecked)).copiedFrom(tp)
        }.copiedFrom(tp)

      case s.TypeBounds(_, s.AnyType()) => ref.copiedFrom(tp)
      case s.TypeBounds(_, upperBound) =>
        refinement(("x" :: ref.copiedFrom(tp)).copiedFrom(tp)) { x =>
          t.Annotated(instanceOf(x, s.AnyType().copiedFrom(tp), upperBound), Seq(t.Unchecked)).copiedFrom(tp)
        }.copiedFrom(tp)

      case tp: s.TypeParameter if testers contains tp =>
        refinement(("x" :: ref.copiedFrom(tp)).copiedFrom(tp)) { x =>
          t.Annotated(instanceOf(x, s.AnyType().copiedFrom(tp), tp), Seq(t.Unchecked)).copiedFrom(tp)
        }.copiedFrom(tp)
      ...
      case tp: s.TypeParameter =>
        super.transform(
          tp.copy(
              flags = tp.flags.filterNot {
                case (_: s.Variance | _: s.Bounds) => true
                case _ => false
              }
            )
            .copiedFrom(tp)
    ...
    def in(id: Identifier): Scope = {
      ...
      val extraTParams = functions
        .get(id)
        .map {
          case FunInfo(fun, tparams) =>
    ...
    override def transform(e: s.Expr, inType: s.Type): t.Expr = e match {
      ...
      case app @ s.ApplyLetRec(id, tparams, tpe, tps, args) =>
        ...
        val nparams = tparams.zipWithIndex.collect {
          case (tp, i) if ctparams(i) =>
        ...
        convert(
          t.ApplyLetRec(
              id,
              tparams.zipWithIndex.collect {
                case (tp, i) if !ctparams(i) => transform(tp).asInstanceOf[t.TypeParameter]
              },
    ...
    private def instanceOfPattern(inner: t.Pattern, in: s.Type, tpe: s.Type): t.Pattern =
      if (isSubtypeOf(in, tpe) && isObject(tpe) == isObject(in)) {
        ...
        t.UnapplyPattern(
            ...
            unapplyAny.id,
            Seq(transform(in), transform(tpe)),
            Seq(inner)
          )
          .copiedFrom(inner)
      }
    ...
    def transform(fd: s.FunAbstraction): t.FunAbstraction = {
      ...
      val (scope, tparamParams) = fd.tparams
        .map(_.tp)
        .zipWithIndex
        ...
        object traverser extends TransformerWithType {
          ...
          override def transform(pat: s.Pattern, in: s.Type): t.Pattern = pat match {
            ...
            case up @ s.UnapplyPattern(ob, recs, id, tps, subs) =>
              ...
              simple --= tps.zipWithIndex.flatMap {
                case (tp, i) =>
    ...
    val result = t.NoSymbols
      ...
      .withSorts(
        Seq(
          new t.ADTSort(
            refID,
            Seq(), // overrides refSort in `independentSymbols.sorts`
