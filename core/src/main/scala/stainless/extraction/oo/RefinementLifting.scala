...
trait RefinementLifting
  ...
  protected class TransformerContext(val symbols: s.Symbols) extends oo.TreeTransformer {
    ...
    def liftRefinements(tpe: s.Type): s.Type =
      s.typeOps.postMap {
        case ft @ s.FunctionType(from, to) =>
          val nfrom = from.map {
            case s.RefinementType(vd, pred) => vd.tpe
            case tpe => tpe
          }
          to match {
            case s.RefinementType(vd, pred) =>
              val nvd =
                s.ValDef(FreshIdentifier("f"), s.FunctionType(nfrom, vd.tpe).copiedFrom(ft), vd.flags).copiedFrom(vd)
        ...
        case s.TupleType(tps) =>
          ...
          if (optPreds.forall(_.isEmpty)) None
          else {
    ...
    def parameterConds(vds: Seq[s.ValDef]): (Seq[s.ValDef], s.Expr) = {
      val (newParams, conds) = vds
        .map(
          vd =>
            liftRefinements(vd.tpe) match {
        ...
        liftRefinements(tpe) match {
          ...
          case s.RefinementType(vd, pred) =>
            transform(
              s.Assert(
                  s.exprOps
                    .freshenLocals(s.exprOps.replaceFromSymbols(Map(vd -> asInstOf(expr, vd.tpe).copiedFrom(e)), pred)),
                  Some("Cast error"),
            ...
            cses.map {
              case cse @ s.MatchCase(pat, guard, rhs) =>
                ...
                val newPat = s.patternOps.postMap {
                  case pat @ s.InstanceOfPattern(ob, tpe) =>
                    liftRefinements(tpe) match {
                      case s.RefinementType(vd, pred) =>
  ...
  override protected def extractFunction(context: TransformerContext, fd: s.FunDef): t.FunDef = {
    ...
    val optPre = cond match {
      case cond if cond != s.BooleanLiteral(true) =>
        s.exprOps.preconditionOf(fd.fullBody) match {
    ...
    val optPost = context.liftRefinements(fd.returnType) match {
      case s.RefinementType(vd2, pred) =>
        s.exprOps.postconditionOf(fd.fullBody) match {
      ...
      Some(sort.invariant match {
        ...
        case None =>
          ...
          mkFunDef(FreshIdentifier("inv"))(sort.typeArgs.map(_.id.name): _*) {
            case tparams =>
              (Seq("thiss" :: s.ADTType(sort.id, tparams).copiedFrom(sort)), s.BooleanType().copiedFrom(sort), {
                case Seq(thiss) =>
                  s.typeOps.instantiateType(
